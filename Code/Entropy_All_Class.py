...
for iteration in range(0,num_Iteration):
    ...
    for class_num in range(0, number_Classes):
        ...
        dataset = pd.read_csv(file_path, header=None)
        dr, dc = dataset.shape
        activation_val_Layer_2 = np.array(
            dataset.iloc[0:dr, 0:dc])  # activation_val stores the activavtion of the neurons in a class in a layer
        ...
        if num_Layer > 2:
            for layers in range(3, num_Layer+1):
                ...
                dataset = pd.read_csv(file_path, header=None)
                dr, dc = dataset.shape
                # activation_val stores the activavtion of the neurons in a class in a layer
    ...
    file_Ps = os.path.join(
        "../Results/Histogram/Entropy_Values_Per_Iteration_" + str(iteration) + ".csv")
    np.savetxt(file_Ps, entropy_Histogram[1:], delimiter=",", fmt='%f', comments="")
    end_time = time.time()
    print('Time :', np.array(end_time) - start_time, 's')
file_P = os.path.join("../Results/Entropy/All_Class/Entropy_Values_Per_Iteration_All_Class.csv")
