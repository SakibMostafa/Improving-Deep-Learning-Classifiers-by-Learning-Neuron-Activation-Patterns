    ...
    if outer_iteration == (iteration * 1):
        ...
        for class_Number in range(num_class):
            ...
            for image in trange(previous_image, (len(train_filenames)) - 1):
                ...
                train_IMAGE = preprocess_input(np.asarray(
                    cv2.resize(cv2.cvtColor(cv2.imread(name), cv2.COLOR_BGR2RGB),
                               (img_width, img_height)), dtype='float32'))
        ...
        file_P = "../Data/Prediction_Results_Iteration_" + str(iteration) + "_.csv"
        np.savetxt(file_P, Final_prediction, delimiter=",", fmt="%f")

        file_P = "../Data/Prediction_Results_Iteration_Test_" + str(iteration) + "_.csv"
